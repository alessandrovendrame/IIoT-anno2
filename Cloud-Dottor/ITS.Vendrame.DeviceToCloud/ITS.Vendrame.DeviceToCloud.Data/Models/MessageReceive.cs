...
namespace ITS.Vendrame.DeviceToCloud.Data.Models
{
    ...
    public class MessageReceive
    {
        ...
        public MessageReceive(DeviceClient deviceClient, IConfiguration configuration)
        {
        ...
        private async Task ReceiveC2dMessagesPollingAndComplete(TimeSpan timeout)
        {
            ...
            {
                Message receivedMessage = await _deviceClient.ReceiveAsync(timeout);
        ...
        private void ProcessReceivedMessage(Message receivedMessage)
        {
            ...
            var splittedString = messageData.Split('_');
            var deviceId = splittedString[1];
            var messageText = splittedString[0];

            var cs = _configuration.GetConnectionString("ItsStorage");
            var storageAccount = CloudStorageAccount.Parse(cs);
            var tableClient = storageAccount.CreateCloudTableClient(new TableClientConfiguration());
            var messagesTable = tableClient.GetTableReference("clientmessagges");

            //messagesTable.CreateIfNotExistsAsync();

            var message = new MessageTableModel(Guid.NewGuid(), deviceId, messageText);
